...
class Adapter(nn.Module, Configurable):
    ...
    def forward(self, *args, check_bounds=False):
        ...
        if check_bounds:
            ...
            for val, pn in zip(
                ...
                [self.pn_gp, self.pn_sfh_disk, self.pn_sfh_bulge]
            ):
                is_out |= pn.check_bounds(val)
            # Apply the parameterisations
            gp, sfh_disk, sfh_bulge = self._apply_pn(gp, sfh_disk, sfh_bulge)
            ...
            return self._apply_pn(gp, sfh_disk, sfh_bulge)
    ...
    def _apply_pn(self, gp, sfh_disk, sfh_bulge):
        gp = self.pn_gp(gp)
